...
- [ ] `pode --help` 显示帮助信息
- [ ] `mypy pode_agent/` 无错误
- [ ] `ruff check pode_agent/` 无错误
- [ ] `pytest tests/` 通过（空测试套件）
...
**文件**：
- `pode_agent/infra/logging.py` — structlog 配置
- `pode_agent/infra/http_client.py` — httpx AsyncClient 工厂
- `pode_agent/infra/fs.py` — 文件系统工具函数
- `pode_agent/infra/shell.py` — Shell 命令执行基础

**验收标准**：
- [ ] 日志可以用 `logger.debug/info/warning/error()` 调用
- [ ] HTTP 客户端支持代理配置
- [ ] 文件系统工具函数有单元测试
...
**文件**：
- `pode_agent/core/config/schema.py` — 所有 Pydantic 数据模型
- `pode_agent/core/config/loader.py` — 读写 config.json
- `pode_agent/core/config/defaults.py` — 默认值
- `pode_agent/core/config/__init__.py` — 公共 API

**验收标准**：
- [ ] `get_global_config()` 在 `~/.pode/config.json` 不存在时返回默认值
- [ ] `save_global_config()` 写入文件，`get_global_config()` 能读回来
- [ ] 配置模型能通过 `model_json_schema()` 生成 JSON Schema
- [ ] 单元测试覆盖读写、默认值、错误处理
